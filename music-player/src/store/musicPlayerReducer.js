...
const repeatMode = ["ALL", "ONE", "SHUFFLE"]
...
export const updatePlaylist = (newPlayList) => ({type: UPDATE_PLAYLIST, newPlayList: newPlayList});

const getRandomNum = (arrLen, excludedNum) => {
    const randomNumber = Math.floor(Math.random() * arrLen);
    return randomNumber === excludedNum? 
        getRandomNum(arrLen, excludedNum) : randomNumber;
}
export default function musicPlayerReducer(state=initialState, action) {
    switch(action.type) {
        ...
        case PREV_MUSIC:
            ...
            getRandomNum(playList.length, state.currentIndex)
            : (state.playList.length + state.currentIndex - 1) % state.playList.length
            return {
                ...state,
